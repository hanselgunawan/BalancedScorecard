    ...
    public partial class delete_objective : System.Web.UI.Page
    {
        ...
        protected void OnClickDelete(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(str_connect))
            {
                ...
                if (counter > 1)
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "redirect", "alert('Delete Specific Objectives:\\n" + sb_deleted_specific_objective + "  \\nSUCCESS!'); window.location='" + baseUrl + "delete_objective.aspx?page=" + page + "&period_id=" + period_id + "&header_id=" + header_id + "';", true);
                }
                else
                {
                    ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "redirect", "alert('There is no selected specific objective to be deleted!'); window.location='" + baseUrl + "delete_objective.aspx?page=" + page + "&period_id=" + period_id + "&header_id=" + header_id + "';", true);
                }
                conn.Close();
            }
        }
