    ...
    public partial class edit_submit_kpi : System.Web.UI.Page
    {
                ...
                using (SqlConnection conn = new SqlConnection(str_connect))
                {
                    ...
                    using (SqlDataReader oReader = sql_select_period.ExecuteReader())
                    {
                        ...
                        else//jika periode tidak ditemukkan / disuntik langsung ke Database
                        {
                            ...
                            cancel_add_new_KPI.Attributes.Add("a href", "view_submit_individual.aspx?page=1&period_id=" + period_id + "&emp_nik=" + emp_nik + "&prev_page=" + prev_page + "");
                        }
                    }
        ...
        protected void OnClickEditKPI(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(str_connect))
            {
                ...
                select_KPI_name = "SELECT IndividualHeader_KPI FROM IndividualMeasures_Header "
                                   + "WHERE IndividualHeader_KPI='" + TextBoxKPI.Value + "' "
                                   + "AND IndividualHeader_ID<>" + header_id + " AND data_status='exist' "
                                   + "AND Period_ID=" + period_id + " AND user_id = " + user_id.ToString() + "";
                ...
                if (KPI_name_exist == false)
                {
                    ...
                    if (DropDownListSpecific.SelectedValue == "No")
                    {
                        ...
                        ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "redirect", "alert('KPI Updated!'); window.location='" + baseUrl + "view_submit_individual.aspx?page=" + page + "&period_id=" + period_id + "&emp_nik=" + emp_nik + "&prev_page=" + prev_page + "';", true);
                    }
