...
{
    public partial class approval_specific_objective : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string get_max_data = "", select_all_detail_request = "", sql_string_active = "";
                ...
                string string_check_access_page = "SELECT Access_Rights_Code FROM GroupAccessRights "//untuk cek, apakah dia boleh akses halaman ini
                                                + "WHERE Access_Rights_Code='approval_specific_objective' AND "//jika diakses secara paksa
                ...
                DropDownListSearchBy.Items.Add("Search By Adt. Organization");
                ...
                using (SqlConnection conn = new SqlConnection(str_connect))
                {
                    ...
                    using (SqlDataReader PeriodReader = sql_command_all.ExecuteReader())//UNTUK VIEW SEMUA PERIODE YANG ADA
                    {
                        ...
                        if (PeriodReader.HasRows)
                        {
                            while (PeriodReader.Read())
                            {
                                ...
                                enddate_to_date = end_date.ToString("MMM yyyy");//ubah format tanggal!
                                start_end_date = startdate_to_date + " - " + enddate_to_date;
                                HtmlDropdown.Append("<li role='presentation'><a role='menuitem' href='approval_specific_objective.aspx?period_id=" + PeriodReader["Period_ID"] + "'>");
                    ...
                    {
                        get_max_data = "SELECT COUNT(IndividualDetailRequest_ID) FROM IndividualDetail_RequestChange WHERE Period_ID=" + period_id + " AND Superior_ID='" + Session["user_nik"] + "'";
                        select_all_detail_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ScorecardUser.EmpId ASC, IndividualDetail_RequestChange.date_create DESC) "
                                                 + "AS rowNum, ScorecardUser.EmpId, OrgName, OrgAdtGroupName, "
                                                 + "empName, IndividualDetailHistory.IndividualHeader_KPI, IndividualDetailHistory.IndividualDetail_Title, "
                                                 + "IndividualDetail_RequestChange.IndividualDetail_ID, "
                                                 + "IndividualDetail_RequestChange.date_create, IndividualDetail_RequestChange.date_update, Approval_Status, IndividualDetail_RequestChange.IndividualDetailRequest_ID, "
                                                 + "IndividualDetail_RequestChange.delete_flag FROM IndividualDetail_RequestChange "
                                                 ...
                                                 + "AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"].ToString() + "')sub "
                    ...
                    else if (req_nik != null && req_name == null && req_org == null && req_adt_org == null)
                    {
                        get_max_data = "SELECT COUNT(IndividualDetailRequest_ID) FROM IndividualDetail_RequestChange "
                                     ...
                                     + "WHERE Period_ID=" + period_id + " AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"] + "' "
                                     + "AND EmpId LIKE '" + req_nik + "%'";
                        select_all_detail_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ScorecardUser.EmpId ASC, IndividualDetail_RequestChange.date_create DESC) "
                                                 + "AS rowNum, ScorecardUser.EmpId, OrgName, OrgAdtGroupName, "
                                                 + "empName, IndividualDetailHistory.IndividualHeader_KPI, IndividualDetailHistory.IndividualDetail_Title, "
                                                 + "IndividualDetail_RequestChange.IndividualDetail_ID, "
                                                 ...
                                                 + "join [Human_Capital_demo].dbo.Organization on ScorecardUser.empOrgCode = Organization.OrgCode "
                                                 + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " "
                                                 + "AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"].ToString() + "' AND ScorecardUser.EmpId LIKE '" + req_nik + "%')sub "
                                                 + "WHERE rowNum>=((" + page + "-1)*" + data_per_page + ")+1 AND rowNum<=" + data_per_page + "*" + page + "";
                        DropDownListSearchBy.SelectedIndex = 0;
                    ...
                    else if (req_nik == null && req_name != null && req_org == null && req_adt_org == null)
                    {
                        get_max_data = "SELECT COUNT(IndividualDetailRequest_ID) FROM IndividualDetail_RequestChange "
                                     + "join ScorecardUser ON ScorecardUser.user_id = IndividualDetail_RequestChange.user_id "
                                     + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"] + "' "
                        ...
                        select_all_detail_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ScorecardUser.EmpId ASC, IndividualDetail_RequestChange.date_create DESC) "
                                                 ...
                                                 + "join [Human_Capital_demo].dbo.Organization on ScorecardUser.empOrgCode = Organization.OrgCode "
                                                 + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " "
                                                 + "AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"].ToString() + "' AND ScorecardUser.empName LIKE '" + req_name + "%')sub "
                                                 + "WHERE rowNum>=((" + page + "-1)*" + data_per_page + ")+1 AND rowNum<=" + data_per_page + "*" + page + "";
                    ...
                    else if (req_nik == null && req_name == null && req_org != null && req_adt_org == null)
                    {
                        ...
                        select_all_detail_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ScorecardUser.EmpId ASC, IndividualDetail_RequestChange.date_create DESC) "
                                                 ...
                                                 + "join [Human_Capital_demo].dbo.Organization on ScorecardUser.empOrgCode = Organization.OrgCode "
                                                 + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " "
                                                 + "AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"].ToString() + "' AND OrgName LIKE '" + req_org + "%')sub "
                                                 + "WHERE rowNum>=((" + page + "-1)*" + data_per_page + ")+1 AND rowNum<=" + data_per_page + "*" + page + "";
                    ...
                    else if (req_nik == null && req_name == null && req_org == null && req_adt_org != null)
                    {
                        ...
                        select_all_detail_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY ScorecardUser.EmpId ASC, IndividualDetail_RequestChange.date_create DESC) "
                                                 ...
                                                 + "join [Human_Capital_demo].dbo.Organization on ScorecardUser.empOrgCode = Organization.OrgCode "
                                                 + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " "
                                                 + "AND IndividualDetail_RequestChange.Superior_ID='" + Session["user_nik"].ToString() + "' AND OrgAdtGroupName LIKE '" + req_adt_org + "%')sub "
                                                 + "WHERE rowNum>=((" + page + "-1)*" + data_per_page + ")+1 AND rowNum<=" + data_per_page + "*" + page + "";
                    ...
                    using (SqlDataReader RequestReader = sql_select_all_detail_request.ExecuteReader())
                    {
                        if (RequestReader.HasRows)
                        {
                            ...
                            HtmlApprovalTable.Append("<th class='centering-th-period'>KPI</th>");
                            ...
                            while (RequestReader.Read())
                            {
                                ...
                                if (req_nik == null && req_name == null && req_org == null && req_adt_org == null)
                                {
                                    HtmlApprovalTable.Append("<td class='td-align'><a href='view_request_specific_objective.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "'>See Details</a></td>");
                                }
                                else if (req_nik != null && req_name == null && req_org == null && req_adt_org == null)
                                {
                                    HtmlApprovalTable.Append("<td class='td-align'><a href='view_request_specific_objective.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "&req_nik=" + req_nik + "'>See Details</a></td>");
                                }
                                else if (req_nik == null && req_name != null && req_org == null && req_adt_org == null)
                                {
                                    HtmlApprovalTable.Append("<td class='td-align'><a href='view_request_specific_objective.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "&req_name=" + req_name + "'>See Details</a></td>");
                                }
                                else if (req_nik == null && req_name == null && req_org != null && req_adt_org == null)
                                {
                                    HtmlApprovalTable.Append("<td class='td-align'><a href='view_request_specific_objective.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "&req_org=" + req_org + "'>See Details</a></td>");
                                }
                                else if (req_nik == null && req_name == null && req_org == null && req_adt_org != null)
                                {
                                    HtmlApprovalTable.Append("<td class='td-align'><a href='view_request_specific_objective.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "&req_adt_org=" + req_adt_org + "'>See Details</a></td>");
                                }
                                ...
                                if (RequestReader["Approval_Status"].ToString() == "pending")
                                {
                                    ...
                                    HtmlApprovalTable.Append("<td class='td-align'><a class='btn btn-default' href='approve_so.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "'>Approve</a></td>");
                                    HtmlApprovalTable.Append("<td class='td-align'><a class='btn btn-default' href='reject_so.aspx?page=" + page + "&request_id=" + RequestReader["IndividualDetailRequest_ID"] + "&period_id=" + period_id + "'>Reject</a></td>");
                                }
        ...
        protected void OnClickSearch(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect(baseUrl + "approval_specific_objective.aspx?period_id=" + period_id + "");
            }
            else
            {
                if (DropDownListSearchBy.SelectedIndex == 0)
                {
                    Response.Redirect(baseUrl + "approval_specific_objective.aspx?req_nik=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 1)
                {
                    Response.Redirect(baseUrl + "approval_specific_objective.aspx?req_name=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 2)
                {
                    Response.Redirect(baseUrl + "approval_specific_objective.aspx?req_org=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 3)
                {
                    Response.Redirect(baseUrl + "approval_specific_objective.aspx?req_adt_org=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
            }
        }
        ...
        protected void OnClickExportExcel(object sender, EventArgs e)
        {
            ...
            using (SqlConnection conn = new SqlConnection(str_connect))
            {
                string string_select_SO_change_request = "SELECT t1.EmpId, t1.empName, t1.OrgName, "
                                                        + "t1.JobTtlName, t1.JobLvlName, t1.OrgAdtGroupName, t1.Group_Name, t1.IndividualHeader_KPI, "
                                                        ...
                                                        + "( "
                                                            ...
                                                            + "JobTtlName, JobLvlName, OrgAdtGroupName, empGrade, Group_Name, "
