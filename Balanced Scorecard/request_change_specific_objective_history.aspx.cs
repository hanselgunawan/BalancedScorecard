...
namespace Balanced_Scorecard
{
    public partial class request_change_specific_objective_history : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string string_check_access_page = "SELECT Access_Rights_Code FROM GroupAccessRights "//untuk cek, apakah dia boleh akses halaman ini
                                                + "WHERE Access_Rights_Code='request_change_specific_objective_history' AND "//jika diakses secara paksa
                ...
                using (SqlConnection conn = new SqlConnection(str_connect))
                {
                    ...
                    if (req_nik == null && req_name == null && req_org == null && req_adt_org == null && app_nik == null && app_name == null)
                    {
                        ...
                        select_all_detail_history = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY CAST(t1.EmpId AS int) ASC, t1.date_create DESC) "
                                                 ...
                                                 + "t1.date_update, t1.OrgName, t1.OrgAdtGroupName, t1.delete_flag FROM "
                    ...
                    else if (req_nik == null && req_name == null && req_org != null && req_adt_org == null && app_nik == null && app_name == null)
                    {
                        get_max_data = "SELECT COUNT(IndividualDetailRequest_ID) FROM IndividualDetail_RequestChange "
                                     + "join ScorecardUser ON IndividualDetail_RequestChange.user_id = ScorecardUser.user_id "
                                     + "join [human_capital_demo].dbo.Organization on ScorecardUser.empOrgCode = Organization.OrgCode "
                                     + "WHERE IndividualDetail_RequestChange.Period_ID=" + period_id + " AND OrgName LIKE '" + req_org + "%'";
                        select_all_detail_history = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY CAST(t1.EmpId AS int) ASC, t1.date_create DESC) "
                                                 + "AS rowNum, t1.IndividualDetail_Title, t1.EmpId, t1.empName, t2.EmpId Superior_NIK, "
                                                 + "t2.empName Superior_Name, t1.IndividualHeader_KPI,  t1.IndividualDetailRequest_ID, t1.Approval_Status, t1.date_create, "
                                                 + "t1.date_update, t1.OrgName, t1.OrgAdtGroupName, t1.delete_flag FROM "
        ...
        protected void OnClickSearch(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?period_id=" + period_id + "");
            }
            else
            {
                if (DropDownListSearchBy.SelectedIndex == 0)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?req_nik=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 1)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?req_name=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 2)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?req_org=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 3)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?req_adt_org=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 4)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?app_nik=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
                else if (DropDownListSearchBy.SelectedIndex == 5)
                {
                    Response.Redirect("" + baseUrl + "request_change_specific_objective_history.aspx?app_name=" + TextBoxSearch.Value + "&period_id=" + period_id + "");
                }
            }
        }
            ...
            using (SqlConnection conn = new SqlConnection(str_connect))
            {
                string string_select_SO_change_request = "SELECT t1.EmpId, t1.empName, t1.OrgName, "
                                                        ...
                                                        + "( "
                                                            ...
                                                            + "IndividualDetail_RequestChange.user_id "
                                                            + "FROM ScorecardUser "
