...
{
    public partial class request_kpi : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                string string_check_access_page = "SELECT Access_Rights_Code FROM GroupAccessRights "//untuk cek, apakah dia boleh akses halaman ini
                                                + "WHERE Access_Rights_Code='request_kpi' AND "//jika diakses secara paksa
                ...
                using (SqlConnection conn = new SqlConnection(str_connect))
                {
                    ...
                    using (SqlDataReader PeriodReader = sql_command_all.ExecuteReader())//UNTUK VIEW SEMUA PERIODE YANG ADA
                    {
                        ...
                        if (PeriodReader.HasRows)
                        {
                            while (PeriodReader.Read())
                            {
                                ...
                                enddate_to_date = end_date.ToString("MMM yyyy");//ubah format tanggal!
                                start_end_date = startdate_to_date + " - " + enddate_to_date;
                                HtmlDropdown.Append("<li role='presentation'><a role='menuitem' href='request_kpi.aspx?period_id=" + PeriodReader["Period_ID"] + "'>");
                    ...
                    string get_max_data = "SELECT COUNT(IndividualHeaderRequest_ID) FROM IndividualHeader_RequestChange WHERE Period_ID=" + period_id + " AND user_id=" + Session["user_id"] + "";
                    string select_all_header_request = "SELECT * FROM (SELECT ROW_NUMBER() OVER(ORDER BY IndividualHeader_RequestChange.IndividualHeaderRequest_ID DESC) "
                                                     + "AS rowNum, IndividualHeaderHistory.IndividualHeader_KPI, IndividualHeader_RequestChange.date_create, "
                                                     + "IndividualHeader_RequestChange.user_update, IndividualHeader_RequestChange.date_update, "
                                                     + "IndividualHeader_RequestChange.IndividualHeaderRequest_ID, Approval_Status, IndividualHeader_RequestChange.delete_flag "
                                                     + "FROM IndividualHeader_RequestChange "
                                                     ...
                                                     + "AND IndividualHeader_RequestChange.user_id=" + Session["user_id"] + ")sub "
                    ...
                    using (SqlDataReader RequestReader = sql_select_all_header_request.ExecuteReader())
                    {
                        if (RequestReader.HasRows)
                        {
                            ...
                            while (RequestReader.Read())
                            {
                                ...
                                HtmlApprovalTable.Append("<td class='td-align'><a href='view_my_kpi_request.aspx?page=" + page + "&request_id=" + RequestReader["IndividualHeaderRequest_ID"] + "&period_id=" + period_id + "'>See Details</a></td>");
